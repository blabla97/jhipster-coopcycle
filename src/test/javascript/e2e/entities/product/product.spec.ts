...
describe('Product e2e test', () => {
  ...
  it('should load Products', async () => {
    ...
    expect(await productComponentsPage.getTitle()).to.eq('coopcycleApp.product.home.title');
  ...
  it('should load create Product page', async () => {
    ...
    expect(await productUpdatePage.getPageTitle()).to.eq('coopcycleApp.product.home.createOrEditLabel');
  ...
  /* it('should create and save Products', async () => {
        ...
        await promise.all([
            ...
            productUpdatePage.productsSelectLastOption(),
            // productUpdatePage.basketsSelectLastOption(),
        ]);
        ...
        const selectedDisponibility = productUpdatePage.getDisponibilityInput();
        if (await selectedDisponibility.isSelected()) {
            await productUpdatePage.getDisponibilityInput().click();
            expect(await productUpdatePage.getDisponibilityInput().isSelected(), 'Expected disponibility not to be selected').to.be.false;
        } else {
            await productUpdatePage.getDisponibilityInput().click();
            expect(await productUpdatePage.getDisponibilityInput().isSelected(), 'Expected disponibility to be selected').to.be.true;
        }
  ...
  /* it('should delete last Product', async () => {
        ...
        expect(await productDeleteDialog.getDialogTitle())
            .to.eq('coopcycleApp.product.delete.question');
